...
class SafetyPointGoal1_time(gymnasium.Env):
    ...
    def step(self, action):
        ...
        # if self.semantics_method_tradition:
        #     final_reward = self.trandition_semantics(goal_dist, hazard_dist)
        # else:
        #     final_reward = self.softmax_semantics(goal_dist, hazard_dist)
        ...
        if truncated or deadline <= 0:
            done = True
            final_reward = -5


class SafetyPointGoal1_delay_withoutTime(gymnasium.Env):
    def __init__(self, config=None, seed=None, render_mode='human', semantics_method_tradition=False):
        # super(SafetyPointGoal1, self).__init__()
        self.total_time = 100
        self.safe_dis = 0.4
